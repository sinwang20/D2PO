...
def export_train_examples(export=True, export_text_samples=True):
        ...
        for e in samples:
            ...
            for s_i, s in enumerate(e['plan']['low_actions']):
                ...
                if nl_skill:
                    ...
                    current_image = None
                    for img in e['images']:
                        if img['low_idx'] == s_i:  # 假设 low_idx 与 s_i 对应
                            current_image = img['image_name']  # 找到对应的图片名称
                            break  # 找到第一个匹配的图片后跳出循环

                    # 将当前图片复制 len(nl_skill) 次，添加到 images 列表
                    if current_image:
                        images.extend([current_image] * len(nl_skill))
    ...
    if export:
        with open('resource/alfred_examples_for_prompt-train_v1.json', 'w') as fp:
            json.dump(selected_samples, fp, indent=4)

    if export_text_samples:
        with open(f'resource/alfred_train_samples_v1.txt', 'w') as fp:
