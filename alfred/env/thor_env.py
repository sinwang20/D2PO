...
class ThorEnv(Controller):
    ...
    def safe_deepcopy(self):
        # Perform deep copy on copyable attributes
        cloned_env = copy.copy(self)  # Shallow copy first
        cloned_env.cleaned_objects = copy.deepcopy(self.cleaned_objects)
        cloned_env.cooled_objects = copy.deepcopy(self.cooled_objects)
        cloned_env.heated_objects = copy.deepcopy(self.heated_objects)

        # Handle Controller-dependent attributes to ensure proper isolation

        # Deep copy last_event (if it is a complex structure)
        # if self.last_event:
        cloned_env.last_event = copy.deepcopy(self.last_event)
        cloned_env.last_action = copy.deepcopy(self.last_action)
        # cloned_env.server = copy.deepcopy(self.server)
        # cloned_env.scene = copy.deepcopy(self.scene)
        # else:
        #     cloned_env.last_event = None

        # Copy scene state (e.g., string or dict describing the scene)
        # cloned_env.scene = copy.deepcopy(self.scene)

        # Handle Unity server and process-specific attributes
        # The server and unity_pid must be re-initialized to avoid shared state

        # Reset Unity process and container IDs, assuming they are unique to each instance
        # cloned_env.unity_pid = copy.deepcopy(self.unity_pid)
        # cloned_env.container_id = copy.deepcopy(self.container_id)

        # Ensure lock_file is a new instance, not shared
        # cloned_env.lock_file = copy.deepcopy(self.lock_file)

        # Reapply initialization parameters for consistency
        # if hasattr(self, "initialization_parameters"):
        #     cloned_env.initialization_parameters = copy.deepcopy(self.initialization_parameters)

        
        # Skip non-copyable attributes like locks and task
        cloned_env.lock = None  # Handle lock here
        cloned_env.task = None  # Direct assignment, assuming task doesn't need deep copy
        # cloned_env.task = self.task.deepcopy()  # Assuming `task` has its own deepcopy method
    ...
    def reset(self, scene_name_or_num,
        ...
        # print("Before calling super().reset()",flush=True)
        super().reset(scene_name)
        # print("After calling super().reset()",flush=True)

        # print("Before calling super().step()",flush=True)
        ...
        # print("After calling super().step()",flush=True)
