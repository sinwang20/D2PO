...
class ThorConnector(ThorEnv):
    def __init__(self, x_display=constants.X_DISPLAY,
        ...
        self.font = ImageFont.truetype("UbuntuMono-B.ttf", 24)
    ...
    def safe_deepcopy(self):
        # First, perform a shallow copy of the parent class to copy common attributes
        cloned_env = super().safe_deepcopy()
        
        # Deep copy or reinitialize attributes specific to ThorConnector
        cloned_env.font = copy.deepcopy(self.font)  # Ensure that font is deep copied if needed
        cloned_env.agent_height = self.agent_height  # Primitive type, just direct copy
        cloned_env.cur_receptacle = copy.deepcopy(self.cur_receptacle)  # Reference, copied as-is
        cloned_env.sliced = copy.deepcopy(self.sliced)  # Simple attribute copy
        
        # Handle reachable_position_kdtree: it needs to be properly reinitialized or deep-copied
        cloned_env.reachable_positions, cloned_env.reachable_position_kdtree = copy.deepcopy(self.get_reachable_positions())
        # cloned_env.last_action = copy.deepcopy(last_action)

        # Also copy any other attributes that may need handling, like those that are None or dynamic
        # cloned_env.reachable_positions = copy.deepcopy(self.reachable_positions)  # Handle if it's an object
