...
from src.alfred.utils import dotdict, load_task_json, ithor_name_to_natural_word, aithor_objects 
...
class AlfredEvaluator(Evaluator):
    ...
    def evaluate(self):
        ...
        for e in splits[cfg.alfred.eval_set]:
            # if 'pick_two_obj_and_place' not in e['task'] and 'clean' not in e['task'] and 'cool' not in e['task'] and 'heat' not in e['task']:
            if 'pick_two_obj_and_place' not in e['task']:
            # if 'movable' in e['task']:
                files.append(e)
    ...
    def evaluate_task(self, env, traj_data, r_idx, model_args, planner, save_path, log_prompt=False, train_gt_steps=None):
        ...
        # closed-loop plan step by step
        prev_steps = []
        prev_action_msg = []
        while not done:
            if t>max_step:
                break
            current_image = Image.fromarray(env.last_event.frame)  # 获取当前状态的图像
            current_image.save(os.path.join(image_save_dir, f'frame_{t}.png'))
            ...
            if self.cfg.alfred.eval_set == 'train' and train_gt_steps is not None:
                ...
                if self.cfg.planner.sft:
                    step, prompt, content = planner.generate_step_details_sft(instruction_text, prev_steps, prev_action_msg, prev_images=imgs, visible_objs_str=env_objects)
                else:
                    step, prompt, content = planner.generate_step_details(instruction_text, prev_steps, prev_action_msg, prev_images=imgs, visible_objs_str=env_objects)
            ...
            log.info(f'Model Response:{content}')
            ...
            try:
                ...
                log.warning(e)
            image_state = env.write_step_on_img(self.cfg.planner.use_predefined_prompt, t+1, action_ret)
            imgs.append(image_state)
            image_state.save(os.path.join(image_save_dir, f'step_{t}.png'))  # 保存步骤图像

            if not action_ret['success']:
                prev_action_msg.append(f"failed because {action_ret['message']}")
            else:
                prev_action_msg.append(f"success {action_ret['message']}")
